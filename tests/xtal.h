...
#define PRINT_FAILED_SIG                                                   \
	printf("%sTest failed with signal %d and status %d %s%s\n\n", RED, \
	       WTERMSIG(status), WEXITSTATUS(status), CROSS, RESET)
#define PRINT_SEG_FAULT                                                        \
	printf("%sTest failed due to segmentation fault %s%s\n\n", RED, CROSS, \
	       RESET)
#define PRINT_FAILED                                         \
	printf("%sTest failed with status %d %s%s\n\n", RED, \
	       WEXITSTATUS(status), CROSS, RESET)
...
void register_test(const char *name, testcase function)
{
	if (test_count >= (register_size - 1)) {
		tests = (test_t *)realloc(
			tests, sizeof(test_t) * (register_size += BUFFER_SIZE));
	}
...
#define ASSERT_EQUAL_LONG(expected, actual)                                  \
	if (expected != actual) {                                            \
		fprintf(stderr, "Assertion failed: expected %ld, got %ld\n", \
			expected, actual);                                   \
		exit(EXIT_FAILURE);                                          \
	}
...
#define ASSERT_EQUAL_STR(expected, actual)                                                                      \
	for (int i = 0; expected[i]; i++) {                                                                     \
		if (expected[i] != actual[i] || !actual[i]) {                                                   \
			fprintf(stderr,                                                                         \
				"Assertion failed: Strings differ at index %d.\nExpected \"%s\", got \"%s\"\n", \
				i, expected, actual);                                                           \
			exit(EXIT_FAILURE);                                                                     \
		}                                                                                               \
	}
