			     ...
			     const UNITY_DISPLAY_STYLE_T style)
{
	...
	} else if ((style & UNITY_DISPLAY_RANGE_UINT) ==
		   UNITY_DISPLAY_RANGE_UINT) {
		UnityPrintNumberUnsigned((UNITY_UINT)number);
	} else {
		UNITY_OUTPUT_CHAR('0');
		UNITY_OUTPUT_CHAR('x');
		UnityPrintNumberHex((UNITY_UINT)number,
				    (char)((style & 0xF) * 2));
	}
}
...
void UnityPrintFloat(const UNITY_DOUBLE input_number)
{
	...
	if (number == 0.0f) {
		...
		decimals =
			((exponent <= 0) && (exponent >= -(sig_digits + 3))) ?
				(-exponent) :
				(sig_digits - 1);
		exponent += decimals;
			      ...
			      const UNITY_FLAGS_T flags)
{
	...
	while ((elements > 0) && (elements--)) {
		...
		case 1:
			expect_val = *(
				UNITY_PTR_ATTRIBUTE const UNITY_INT8 *)expected;
			actual_val =
				*(UNITY_PTR_ATTRIBUTE const UNITY_INT8 *)actual;
		...
		case 2:
			expect_val = *(UNITY_PTR_ATTRIBUTE const UNITY_INT16 *)
					     expected;
			actual_val = *(
				UNITY_PTR_ATTRIBUTE const UNITY_INT16 *)actual;
			if (style & (UNITY_DISPLAY_RANGE_UINT |
				     UNITY_DISPLAY_RANGE_HEX)) {
		...
		case 8:
			expect_val = *(UNITY_PTR_ATTRIBUTE const UNITY_INT64 *)
					     expected;
			actual_val = *(
				UNITY_PTR_ATTRIBUTE const UNITY_INT64 *)actual;
		...
		case 4:
			expect_val = *(UNITY_PTR_ATTRIBUTE const UNITY_INT32 *)
					     expected;
			actual_val = *(
				UNITY_PTR_ATTRIBUTE const UNITY_INT32 *)actual;
#ifdef UNITY_SUPPORT_64
			if (style & (UNITY_DISPLAY_RANGE_UINT |
				     UNITY_DISPLAY_RANGE_HEX)) {
		...
		if (flags == UNITY_ARRAY_TO_ARRAY) {
			expected = (UNITY_INTERNAL_PTR)((const char *)expected +
							increment);
		}
	...
	const UNITY_LINE_TYPE lineNumber, const UNITY_FLAGS_T flags)
{
	...
	while (elements--) {
		...
		if (!UnityFloatsWithin(in_delta + current_element_delta,
				       *ptr_expected, *ptr_actual)) {
			UnityTestResultsFailBegin(lineNumber);
			UnityPrint(UnityStrElement);
			UnityPrintNumberUnsigned(num_elements - elements - 1);
			UNITY_PRINT_EXPECTED_AND_ACTUAL_FLOAT(
				(UNITY_DOUBLE)*ptr_expected,
				   ...
				   const UNITY_LINE_TYPE lineNumber)
{
	...
	if ((compare & UNITY_EQUAL_TO) &&
	    UnityFloatsWithin(threshold * UNITY_FLOAT_PRECISION, threshold,
			      actual)) {
		failed = 0;
	}
	...
	const UNITY_LINE_TYPE lineNumber, const UNITY_FLAGS_T flags)
{
	...
	while (elements--) {
		...
		if (!UnityDoublesWithin(in_delta + current_element_delta,
					*ptr_expected, *ptr_actual)) {
			UnityTestResultsFailBegin(lineNumber);
			UnityPrint(UnityStrElement);
			UnityPrintNumberUnsigned(num_elements - elements - 1);
			UNITY_PRINT_EXPECTED_AND_ACTUAL_FLOAT(*ptr_expected,
							      *ptr_actual);
			UnityAddMsgIfSpecified(msg);
			UNITY_FAIL_AND_BAIL;
		}
	...
	if ((style & UNITY_DISPLAY_RANGE_INT) == UNITY_DISPLAY_RANGE_INT) {
		if (actual > expected) {
			Unity.CurrentTestFailed =
				(((UNITY_UINT)actual - (UNITY_UINT)expected) >
				 delta);
		} else {
			Unity.CurrentTestFailed =
				(((UNITY_UINT)expected - (UNITY_UINT)actual) >
				 delta);
		}
	} else {
	...
	const UNITY_DISPLAY_STYLE_T style, const UNITY_FLAGS_T flags)
{
	...
	while ((elements > 0) && (elements--)) {
		...
		case 1:
			...
			if ((style & UNITY_DISPLAY_RANGE_INT) ==
			    UNITY_DISPLAY_RANGE_INT) {
				expect_val = *(
					UNITY_PTR_ATTRIBUTE const UNITY_INT8 *)
						     expected;
				actual_val = *(
					UNITY_PTR_ATTRIBUTE const UNITY_INT8 *)
						     actual;
				...
				expect_val =
					(UNITY_INT) *
					(UNITY_PTR_ATTRIBUTE const UNITY_UINT8 *)
						expected;
				actual_val =
					(UNITY_INT) *
					(UNITY_PTR_ATTRIBUTE const UNITY_UINT8 *)
		...
		case 2:
			...
			if ((style & UNITY_DISPLAY_RANGE_INT) ==
			    UNITY_DISPLAY_RANGE_INT) {
				expect_val = *(
					UNITY_PTR_ATTRIBUTE const UNITY_INT16 *)
						     expected;
				actual_val = *(
					UNITY_PTR_ATTRIBUTE const UNITY_INT16 *)
						     actual;
				...
				expect_val =
					(UNITY_INT) *
					(UNITY_PTR_ATTRIBUTE const UNITY_UINT16
						 *)expected;
				actual_val =
					(UNITY_INT) *
					(UNITY_PTR_ATTRIBUTE const UNITY_UINT16
		...
		case 8:
			...
			if ((style & UNITY_DISPLAY_RANGE_INT) ==
				...
				expect_val =
					(UNITY_INT) *
					(UNITY_PTR_ATTRIBUTE const UNITY_UINT64
						 *)expected;
				actual_val =
					(UNITY_INT) *
					(UNITY_PTR_ATTRIBUTE const UNITY_UINT64
		...
		case 4:
			...
			if ((style & UNITY_DISPLAY_RANGE_INT) ==
			    UNITY_DISPLAY_RANGE_INT) {
				expect_val = *(
					UNITY_PTR_ATTRIBUTE const UNITY_INT32 *)
						     expected;
				actual_val = *(
					UNITY_PTR_ATTRIBUTE const UNITY_INT32 *)
						     actual;
				...
				expect_val =
					(UNITY_INT) *
					(UNITY_PTR_ATTRIBUTE const UNITY_UINT32
						 *)expected;
				actual_val =
					(UNITY_INT) *
					(UNITY_PTR_ATTRIBUTE const UNITY_UINT32
		...
		if ((style & UNITY_DISPLAY_RANGE_INT) ==
			...
			if (actual_val > expect_val) {
				Unity.CurrentTestFailed =
					(((UNITY_UINT)actual_val -
					  (UNITY_UINT)expect_val) > delta);
			} else {
				Unity.CurrentTestFailed =
					(((UNITY_UINT)expect_val -
					  (UNITY_UINT)actual_val) > delta);
			}
		} else {
	...
	while (elements--) {
		...
		while (bytes--) {
			if (*ptr_exp != *ptr_act) {
				...
				if (num_elements > 1) {
					UnityPrint(UnityStrElement);
					UnityPrintNumberUnsigned(num_elements -
								 elements - 1);
				}
				...
				UnityPrintNumberByStyle(
					*ptr_exp, UNITY_DISPLAY_STYLE_HEX8);
				UnityPrint(UnityStrWas);
				UnityPrintNumberByStyle(
					*ptr_act, UNITY_DISPLAY_STYLE_HEX8);
				UnityAddMsgIfSpecified(msg);
				UNITY_FAIL_AND_BAIL;
			}
			ptr_exp++;
		...
		if (flags == UNITY_ARRAY_TO_VAL) {
			ptr_exp = (UNITY_PTR_ATTRIBUTE const unsigned char *)
				expected;
		}
	}
}
	...
	for (i = 1; i < argc; i++) {
		if (argv[i][0] == '-') {
			...
			case 'f': /* an alias for -n */
				UnityStrictMatch =
					(argv[i][1] ==
					 'n'); /* strictly match this string if -n */
				if (argv[i][2] == '=') {
				  ...
				  const char *shortstring)
{
	...
	while (*lptr) {
		...
		while (*lptr && *sptr && (*lptr == *sptr)) {
			...
			case '\'':
			case 0:
				return (!UnityStrictMatch || (*lptr == 0)) ? 1 :
									     0;
...
static int UnityStringArgumentMatches(const char *str)
{
	...
	while (ptr1[0] != 0) {
		...
		if ((retval == 2) && (ptrf != 0)) {
			if (IsStringInBiggerString(Unity.CurrentTestName,
						   ptrf)) {
				return 1;
			}
		}
